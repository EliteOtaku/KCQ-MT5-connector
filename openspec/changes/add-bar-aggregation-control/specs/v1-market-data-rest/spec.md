...
高周期重采样。`aligned` SHALL 在连接器对齐配置允许（`ALIGN_UTC=on`）时，按 UTC 自然边界
重采样；配置不允许时 SHALL 返回 `UNSUPPORTED_CAPABILITY`，不得返回 `original` 数据冒充 `aligned`。
...
- **WHEN** 请求 `period=daily`、`barAggregation=aligned` 且 `ALIGN_UTC=on`
- **THEN** 系统从 MT5 H1 周期读取并按 UTC 自然边界重采样
- **THEN** 响应 `barAggregation` 为 `aligned`

#### Scenario: 对齐模式不可用

- **WHEN** 请求 `barAggregation=aligned` 且 `ALIGN_UTC=off`
